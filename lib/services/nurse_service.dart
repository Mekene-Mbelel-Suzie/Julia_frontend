...
class NurseService {
  ...
  Future<List<NurseChildModel>> getHospitalChildren() async {
    ...
    if (response.statusCode == 200) {
      return (data as List)
          .map((item) => NurseChildModel.fromJson(item))
          .toList();
    }
  ...
  Future<List<NurseDashboardRecordModel>> getDashboardRecords({
    String? tab,
    String? search,
  }) async {
    final uri = Uri.parse('$baseUrl/nurse/dashboard-records/').replace(
      queryParameters: {
    ...
    if (response.statusCode == 200) {
      return (data as List)
          .map((item) => NurseDashboardRecordModel.fromJson(item))
          .toList();
    }
