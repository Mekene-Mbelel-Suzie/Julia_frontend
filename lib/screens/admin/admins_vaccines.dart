        ...
        SnackBar(
          content: Row(
            children: [
              ...
              Expanded(child: Text('Failed to load vaccines: $e')),
            ],
          ),
  ...
  List<VaccineModel> get _filtered {
    ...
    return vaccines.where((v) {
      return v.name.toLowerCase().contains(q) ||
          ...
          (v.diseasePrevented ?? '').toLowerCase().contains(q);
    }).toList();
  }
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  AdminPageHeader(
                    title: 'Vaccines',
                    subtitle: 'Define vaccines used in the vaccination program.',
                  ...
                  AdminSearchBar(
                    ...
                    hint: 'Search by name, code, or disease…',
                    onSearch: () => setState(() {}),
                    accent: _accent,
                  ),
                  const SizedBox(height: 24),
          ...
          if (isLoading)
            ...
            const SliverFillRemaining(
              child: AdminEmptyState(
                ...
                subtitle: 'Add a vaccine to start building schedules.',
                color: _accent,
              ),
            )
          else
                    ...
                    child: SlideTransition(
                      ...
                      child: Padding(
                        ...
                        child: _VaccineCard(
                          vaccine: filtered[i],
                          isLight: isLight,
                          onEdit: () => openForm(vaccine: filtered[i]),
                        ),
                      ),
                    ),
                  ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if ((v.diseasePrevented ?? '').trim().isNotEmpty)
                      Text(
                        'Prevents: ${v.diseasePrevented}',
                        style: TextStyle(
                          fontSize: 12.5,
                          color: isLight
...
class _VaccineFormDialogState extends State<_VaccineFormDialog> {
  ...
  late final TextEditingController diseaseCtrl;
  late final TextEditingController dosesCtrl;
  ...
  void initState() {
    ...
    diseaseCtrl = TextEditingController(text: widget.vaccine?.diseasePrevented ?? '');
    dosesCtrl = TextEditingController(text: widget.vaccine?.numberOfDoses.toString() ?? '1');
  ...
  Future<void> _submit() async {
    ...
    try {
      final doses = int.tryParse(dosesCtrl.text.trim()) ?? 1;
      ...
      if (widget.vaccine == null) {
        await adminService.createVaccine(
          ...
          diseasePrevented: diseaseCtrl.text.trim(),
          numberOfDoses: doses,
          isActive: isActive,
        ...
        child: Column(
          children: [
            ...
            AdminActiveToggle(
              ...
              accent: _accent,
              activeLabel: 'Vaccine is available for scheduling',
              inactiveLabel: 'Vaccine is not available',
            ),
            const SizedBox(height: 4),
          ],
        ),
