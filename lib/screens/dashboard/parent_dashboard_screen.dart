...
class _ParentDashboardScreenState extends State<ParentDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    final pages = [_overview(), const ParentChildrenScreen()];
    return DashboardShell(
      role: 'Parent', accentColor: const Color(0xFFF57C00), user: widget.user,
  ...
  Widget _overview() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        WelcomeBanner(user: u, accent: const Color(0xFFF57C00), subtitle: 'Track your children, follow vaccination plans, and never miss an upcoming dose.'),
        ...
        Wrap(spacing: 12, runSpacing: 12, children: [
          _ActionCard(icon: Icons.child_care_rounded, label: 'My Children', description: 'Add a child and generate a vaccination plan.', color: const Color(0xFFF57C00), onTap: () => setState(() => _idx = 1), isLight: isLight),
          _ActionCard(icon: Icons.vaccines_rounded, label: 'Vaccination Plans', description: 'Track upcoming and past doses for each child.', color: const Color(0xFF0288D1), onTap: () => setState(() => _idx = 1), isLight: isLight),
        ]),
      ]),
    );
  }
...
class _ProfileTable extends StatelessWidget {
  final AppUser user;
  final bool isLight;
  const _ProfileTable({required this.user, required this.isLight});
      ...
      child: Table(
        ...
        children: rows.asMap().entries.map((e) => TableRow(
          ...
          children: [
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 13),
                child: Text(e.value.$2, style: TextStyle(fontSize: 13.5, color: isLight ? const Color(0xFF334155) : const Color(0xFFCBD5E1)))),
          ],
        )).toList(),
      ),
    );
...
class _ActionCard extends StatefulWidget {
  ...
  final bool isLight;
  const _ActionCard({required this.icon, required this.label, required this.description, required this.color, required this.onTap, required this.isLight});
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          width: 240, padding: const EdgeInsets.all(18),
          decoration: BoxDecoration(
          ...
          child: Row(children: [
            Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: widget.color.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
                child: Icon(widget.icon, color: widget.color, size: 20)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(widget.label, style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: widget.isLight ? const Color(0xFF0D1B2A) : Colors.white)),
              const SizedBox(height: 3),
              Text(widget.description, style: const TextStyle(fontSize: 11.5, color: Color(0xFF94A3B8)), maxLines: 2),
            ])),
          ]),
        ),
      ),
...
Widget _SectionLabel(String text, bool isLight) => Text(
  text,
